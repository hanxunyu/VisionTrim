...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return input_ids, image_tensor, image.size, original_qs
...
def collate_fn(batch):
    input_ids, image_tensors, image_sizes, original_qs = zip(*batch)
    input_ids = torch.stack(input_ids, dim=0)
    image_tensors = torch.stack(image_tensors, dim=0)
    return input_ids, image_tensors, image_sizes, original_qs
...
def eval_model(args):
    ...
    # print(f"the model_name is {model_name}")
    # print(f"the model_base ins {args.model_base}")
    ...
    IDX = 1
    for (input_ids, image_tensor, image_sizes, original_qs), line in tqdm(zip(data_loader, questions), total=len(questions)):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--DVTS_token_num", type=int, default=48)
    parser.add_argument("--TGVC_token_num", type=int, default=16)
