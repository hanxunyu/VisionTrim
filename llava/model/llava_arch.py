...
class LlavaMetaForCausalLM(ABC):
    ...
    def encode_images(self, images, original_qs=None):
        ...
        if self.get_model().method == "VisionTrim":
            DVTS_token_num = self.get_model().DVTS_token_num
            TGVC_token_num = self.get_model().TGVC_token_num
            cls_attn, image_features = self.get_model().get_vision_tower()(images, method=self.get_model().method, dataset_name=dataset_name, start_layer=None, DVTS_token_num=DVTS_token_num, TGVC_token_num=TGVC_token_num, original_qs=original_qs)
        else:
